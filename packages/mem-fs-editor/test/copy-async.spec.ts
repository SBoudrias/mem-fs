...
describe.each(['copy', 'copyAsync'] as const)('#%s()', (method) => {
  ...
  it(`${method === 'copy' ? "doesn't return" : 'returns'} a promise`, () => {
  ...
  it('copy by mixed file and glob sources', async () => {
    ...
    expect(memFs.read(path.join(outputDir, 'file-a.txt'))).toBe(`foo${os.EOL}`);
    expect(memFs.read(path.join(outputDir, 'file-tpl.txt'))).toBe(`<%= name %>${os.EOL}`);
  });
  ...
  it('transforms files when globbing', async () => {
    // 10 total files under 'fixtures', not counting folders
    if (method === 'copy') {
      const syncTransform = vi
        .fn<
          NonNullable<NonNullable<Parameters<MemFsEditor['copy']>[2]>['fileTransform']>
        >()
        .mockImplementation(({ destinationPath, contents }) => ({
          path: destinationPath,
          contents,
        }));
      memFs.copy(getFixture('**'), outputDir, { fileTransform: syncTransform });
      expect(syncTransform).toHaveBeenCalledTimes(13);
    } else {
      const asyncTransform = vi
        .fn<
          NonNullable<
            NonNullable<Parameters<MemFsEditor['copyAsync']>[2]>['fileTransform']
          >
        >()
        .mockImplementation(({ destinationPath, contents }) =>
      ...
      await memFs.copyAsync(getFixture('**'), outputDir, {
        fileTransform: asyncTransform,
      });
      expect(asyncTransform).toHaveBeenCalledTimes(13);
    }
  ...
  it('detects fromBasePath from common prefix', async () => {
    await memFs[method](
      [getFixture('file-a.txt'), getFixture('nested/file.txt')],
      outputDir,
      {
        noGlob: true,
      },
    ...
    if (method === 'copy') {
      memFs.copy(filepath, newPath, {
        fileTransform({ destinationPath, sourcePath, contents, options, data }) {
          assertParameters({ destinationPath, sourcePath, contents, options, data });
      ...
      await memFs.copyAsync(filepath, newPath, {
        fileTransform({ destinationPath, sourcePath, contents, options, data }) {
          assertParameters({ destinationPath, sourcePath, contents, options, data });
          // Return unmodified path and content
          return Promise.resolve({ path: destinationPath, contents });
        },
        transformData,
        transformOptions,
      });
